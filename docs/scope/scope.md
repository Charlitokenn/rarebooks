...
- [ ] Build it: `/develop tenant schema & data layer` (in progress, partial)
  ...
  - [ ] Generic doc CRUD routes in `worker/routes/`, behind feature 06's tenant resolution middleware — not started. Real new risk found while building the step above, worth reading before starting: `fyo/model/doc.ts` (the base `Doc` class every model uses) has a non type-only import of `src/utils/erpnextSync.ts`, a Desktop/Electron-only file (references `ipc`, `Navigator.onLine`). It didn't break anything here because `DatabaseCore` never touches `Doc` instances and esbuild tree-shook it out of the actual bundle, confirmed empirically — but CRUD routes built on `fyo`'s `Doc`/model layer, as the spec calls for, may reach this import for real. Worth an explicit check before assuming the same tree-shaking holds once real `Doc` methods are in the call path.
  - [ ] Migration runner utility over every `tenant_projects` row — not started; still needs an invocation mechanism decided (HTTP route vs. Cron Trigger vs. one-off script), which the spec doesn't yet name.
