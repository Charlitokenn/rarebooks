...
- [ ] Build it: `/develop web app shell`
    ...
    - [x] Platform guards on Electron-only actions, disabled sidebar entries, desktop regression pass, satisfies AC-8, AC-10, AC-7
      Guards and disabled entries built 2026-09-15 (all touched raw `ipc` moved to `fyo/demux/shell.ts`). Regression verified 2026-09-18: desktop build `✓ built in 3m 22s` (no web shell code in dist_electron/build/*.js); typecheck/lint add zero new errors (tsc 75, vue-tsc 109, baseline); desktop dev-launch pending (GUI session needed).
...
- [ ] Verify it: `/check verify web app shell` (command-based checks all pass; UI/manual tests blocked on Clerk credentials and READY tenant provisioned on Neon)
