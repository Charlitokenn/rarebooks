...
| Shared: `custom/web/payments/paypal-client.ts` (token, create, cancel, get, verify REST calls), `custom/web/billing/plans.ts`, `custom/web/billing/types.ts` (new status union, new wire constants), `custom/web/billing/subscriptionGate.ts` (`gateDecision(status, callKind)` returning allow / read-only / block, used by the middleware) | | | | | |
